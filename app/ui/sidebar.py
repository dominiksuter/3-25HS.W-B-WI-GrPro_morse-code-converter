...
class Sidebar:
    ...
    def __init__(
        self, service: ChatService, active_chat_id: str | None
    ) -> None:
    ...
    def _render_chat_row(self, chat) -> None:
        ...
        with (
            ui.element("div")
            .classes(row_class)
            .on("click", lambda c=chat: ui.navigate.to(f"/chat/{c.id}"))
        ):
    ...
    def _toggle_pin(self, chat_id: str) -> None:
        ...
        ui.navigate.to(
            f"/chat/{chat_id}" if chat_id == self.active_chat_id else "/"
        )

    def _rename_chat(self, chat_id: str, current_title: str) -> None:
        with ui.dialog() as dialog, ui.card().style("min-width: 420px;"):
            ui.label("Chat umbenennen").style(
                "font-size: 1.05rem; font-weight: 600;"
            )
            title_input = ui.input(
                value=current_title, placeholder="Name eingeben …"
            ).props("autofocus")
            with ui.row().classes("justify-end w-full"):
                ui.button("Abbrechen", on_click=dialog.close).props(
                    "flat no-caps"
                )
                ...
                def save() -> None:
                    ...
                    dialog.close()
                    ui.navigate.to(
                        f"/chat/{self.active_chat_id}"
                        if self.active_chat_id
                        else "/"
                    )

                ui.button("Speichern", on_click=save).props(
                    "unelevated no-caps"
                )
        dialog.open()
